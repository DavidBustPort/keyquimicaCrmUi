...
internal static class ProductOperations
{
    ...
    internal static readonly string SaveSql = ProjectSql.Transaction(ProjectSql.OwnedOpen + """
        ...
        DECLARE @ProductsXml xml = CAST(@Products AS xml);
        DECLARE products CURSOR LOCAL FAST_FORWARD FOR
            SELECT item.value('(@Sku)[1]', 'int'), item.value('(@Cantidad)[1]', 'int'),
                item.value('(@PrecioVentaSugerido)[1]', 'float'), item.value('(@DeleteProducto)[1]', 'bit')
            FROM @ProductsXml.nodes('/items/item') AS entries(item);
        OPEN products;
    ...
    internal static Task<int> Save(ISianwebDbContext db,long cd,int rik,int id,List<ProductosRequest> products,CancellationToken ct) =>
        db.ExecuteAsync(SaveSql,new { Cd=cd,Rik=rik,IdOportunidad=id,Products=new XElement("items", products.Select(p => new XElement("item",
            new XAttribute("Sku", p.Sku), new XAttribute("Cantidad", p.Cantidad),
            new XAttribute("PrecioVentaSugerido", p.PrecioVentaSugerido), new XAttribute("DeleteProducto", p.DeleteProducto ? 1 : 0)))).ToString(SaveOptions.DisableFormatting) },ct);
}
