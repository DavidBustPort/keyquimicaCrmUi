...
export class AppSidebar {
	...
	readonly visibleItems = computed(() =>
		this.items.filter((item) => {
			const routes = this.router.config.find((route) => route.path === '' && route.children)?.children ?? []
			const route = routes.find((route) => route.path === item.path.slice(1))
			const modes = route?.data?.['modes'] as string[] | undefined
			return !modes || (this.auth.isFullyAuthenticated() && modes.includes(this.auth.isCentral() ? 'central' : 'sucursal'))
		})
	)
	...
	readonly reports = [
		...
		{ label: 'Tracking de proyectos cerrados', path: '/tracking-cerrados' },
