...
export class Oportunidades {
    ...
    readonly client = computed(
        () => this.store.clientes().find((c) => c.id === this.clientId()) ?? null,
    )
    readonly choices = computed(() =>
        this.store.clientes().map((c) => ({
            ...
            description:
                ...
                c.id +
                ' · ' +
                (c.tipoClienteId === '2' ? 'Cliente' : 'Prospecto') +
                ' · ' +
                (c.registro ? 'Registro completo' : 'Completa sus datos antes de continuar'),
        })),
    )
    readonly projects = computed(() =>
        this.store.proyectos().filter((p) => p.prospectoId === this.clientId()),
    )
    readonly projectRows = computed(() =>
        this.projects().slice((this.page() - 1) * this.size(), this.page() * this.size()),
    )
    ...
    readonly enabled = computed(
        () =>
            !!this.client()?.registro && (this.store.dimensiones()[this.clientId() ?? 0] ?? 0) > 0,
    )
