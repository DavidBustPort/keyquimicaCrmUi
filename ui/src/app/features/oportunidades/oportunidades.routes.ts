...
export const oportunidadesRoutes: Routes = [
	{
		...
		data: { breadcrumb: 'Oportunidades de Proyectos', modes: ['sucursal'] },
		children: [
			{
				path: '',
				pathMatch: 'full',
				loadComponent: () => import('./pages/embudo-list/embudo-list').then((m) => m.EmbudoList),
				data: { breadcrumb: 'Embudo de Proyectos', canExpandContainer: false, expandedByDefault: true, modes: ['sucursal'], showFilterByRik: true },
				title: 'Embudo de Proyectos | CRM V3'
			},
			...['oportunidades/:prospectoId/:oportunidadId', 'oportunidades/:prospectoId', 'oportunidades'].map((path) => ({
				path,
				loadComponent: opportunity,
				canDeactivate: [(component: Oportunidades) => component.confirmNavigation()],
				data: { breadcrumb: 'Oportunidades', canExpandContainer: false, expandedByDefault: true },
