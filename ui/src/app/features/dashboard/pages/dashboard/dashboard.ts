...
@Component({
	...
	imports: [CurrencyPipe, DecimalPipe, DashboardFilters, DashboardMetrics, DashboardStatistics, ProgressGauge],
...
export class Dashboard {
	...
	readonly sourceGradient = computed(() => {
		...
		return total ? 'conic-gradient(#00a3ff 0% ' + (this.data().sources[0].count / total) * 100 + '%, #a855f7 ' + (this.data().sources[0].count / total) * 100 + '% 100%)' : '#e8edf4'
	})
