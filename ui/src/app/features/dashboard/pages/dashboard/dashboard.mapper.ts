...
import { EtapasOportunidadesName, EtapasOportunidadesColor } from '@app/core/models/etapas-oportunidades.model'

export function mapDashboard(raw: ApiData | undefined): DashboardData {
	const stages = STAGES.map((stage, i) => {
		const current = raw?.estadisticasPorEtapa?.find((e) => e.etapa === i + 1 && e.periodo === 'Actual')
		const previous = raw?.estadisticasPorEtapa?.find((e) => e.etapa === i + 1 && e.periodo === 'TresMesesAtras')
		return {
			...stage,
	...
	return {
		...
		metricAmounts: [raw?.totalesPorEtapas?.analisis ?? 0, raw?.totalesPorEtapas?.promocion ?? 0, raw?.totalesPorEtapas?.negociacion ?? 0],
		newProjectsGoal: raw?.cumplimientoPresupuestoProyectosNuevos?.metaMensualProyectosNuevos ?? 0,
