...
export class DashboardService {
    ...
    readonly dashboardResource = resource({
        params: () => {
            ...
            return this.auth.isManager()
                ? { ...query, RiksId: this.rikFilter.selectedRikId()?.toString() ?? null }
                : query
        },
        loader: ({ params }) =>
            this.loadingService.wrap(firstValueFrom(this.apiService.getDashboardData(params))),
    })
    ...
    async group(group: string) {
        ...
        try {
            ...
            if (version === this.catalogVersion)
                this.error.set(
                    'No se pudieron cargar las sucursales. Vuelve a seleccionar el grupo.',
                )
        } finally {
    ...
    private async loadRiks(branch: number | null, selectAll: boolean) {
        ...
        try {
            ...
            if (version === this.catalogVersion)
                this.error.set('No se pudieron cargar los RIKs. Vuelve a seleccionar la sucursal.')
        } finally {
            if (version === this.catalogVersion) this.catalogLoading.set(false)
        }
    }
    ...
    buildQuery(): DashboardQuery {
        ...
        return {
            ...
            RiksId: this.auth.isCentral()
                ? f.representatives.join(',') || null
                : this.auth.isManager()
                  ? (this.rikFilter.selectedRikId()?.toString() ?? null)
                  : null,
        }
    }
    apply() {
        ...
        const f = this.filter()
        if (
            ![f.start, f.end].every((d) => /^[1-9]\d{3}-(0[1-9]|1[0-2])$/.test(d)) ||
            f.start > f.end
        ) {
