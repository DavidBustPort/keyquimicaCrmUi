...
export class ProspectosLeadsDemoStore {
    ...
    private active(id: number) {
        ...
        if (lead.estado === 'Desarrollado' || lead.estado === 'Rechazado')
            throw new Error('Este lead ya fue desarrollado o rechazado.')
        return lead
    }
    private manager() {
        if (this.view() !== 'gerente')
            throw new Error('Esta acción está disponible en la vista de gerente.')
    }
    changeBranch(id: number, sucursalId: string) {
        ...
        this.leads.update((rows) =>
            rows.map((item) =>
                item.id === id
                    ? { ...item, sucursalId, representanteId: '', estado: 'Pendiente' }
                    : item,
            ),
        )
    ...
    assign(id: number, representanteId: string) {
        ...
        if (
            !REPRESENTANTES.some(
                (item) => item.value === representanteId && item.sucursalId === lead.sucursalId,
            )
        )
            throw new Error('Selecciona un representante de la sucursal del lead.')
        this.leads.update((rows) =>
            rows.map((item) =>
                item.id === id ? { ...item, representanteId, estado: 'Disponible' } : item,
            ),
        )
    ...
    reject(id: number, motivo: string, actor: 'gerente' | 'representante' = 'gerente') {
        ...
        if (
            actor === 'representante' &&
            (lead.estado !== 'Disponible' || lead.representanteId !== this.representanteActual)
        )
            throw new Error('Este lead no está asignado al representante actual.')
        if (!motivo.trim()) throw new Error('Especifica el motivo de rechazo.')
        this.leads.update((rows) =>
            rows.map((item) =>
                item.id === id
                    ? { ...item, estado: 'Rechazado', motivo: motivo.trim(), rechazadoPor: actor }
                    : item,
            ),
        )
    ...
    canDevelop(id: number) {
        return this.leads().some(
            (lead) =>
                ...
                lead.representanteId === this.representanteActual,
        )
    }
    develop(id: number) {
        if (!this.canDevelop(id))
            throw new Error('Este lead ya no está disponible para el representante actual.')
        this.leads.update((rows) =>
            rows.map((item) => (item.id === id ? { ...item, estado: 'Desarrollado' } : item)),
        )
    }
}
