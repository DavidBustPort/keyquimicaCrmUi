...
import { catchError, firstValueFrom, of, Subject, Observable } from 'rxjs'
@Injectable()
export class LeadsService {
	...
	constructor() {
		this.requests
			.pipe(
				latestSearch(
					(q) => (q ? (q.filter ?? '') : null),
					(query) => (query ? this.api.getLeads(query).pipe(catchError((error) => of({ error }))) : of(null))
				),
				takeUntilDestroyed()
			)
			.subscribe((result) => {
				...
				if (!result) return
				if ('error' in result) {
					this.error.set(result.error instanceof Error ? result.error.message : 'No se pudieron cargar los leads.')
					return
				}
		...
		effect(() => {
			...
			const scope = JSON.stringify([allowed, manager, session.userId, session.sucursalId, rik])
	...
	async loadCatalog(mode: 'sucursal' | 'representante') {
		...
		const rows = await firstValueFrom(mode === 'sucursal' ? this.api.getSucursales() : this.api.getRiks())
	...
	private requireManager() {
		if (!this.allowed() || !this.auth.isManager()) throw new Error('Esta acción solo está disponible para el gerente de sucursal.')
	}
	async changeBranch(id: number, value: string) {
		this.requireManager()
		if (!this.branches().some((b) => String(b.id) === value)) throw new Error('Selecciona una sucursal válida.')
		if (this.selected()?.sucursalId === value) throw new Error('Selecciona una sucursal diferente a la actual.')
		await this.save(this.api.changeBranch(id, Number(value)), 'Sucursal actualizada correctamente.')
	}
	async assign(id: number, value: string) {
		this.requireManager()
		if (!this.reps().some((r) => String(r.id) === value)) throw new Error('Selecciona un RIK válido.')
	...
	async reject(id: number, type: number, reason: string) {
		...
		if (type === 3 && (!reason.trim() || reason.trim().length > 500)) throw new Error('Especifica un motivo de entre 1 y 500 caracteres.')
		await this.save(this.api.reject(id, type, type === 3 ? reason.trim() : null), 'Lead rechazado correctamente.')
	}
